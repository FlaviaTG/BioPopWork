library(GenWin)
library(dplyr)
data <- read.table("newH-genolike2-greenjay_ZW-test.vcf-100bp.windowed.weir.fst", header=T)
#subset one region/chromosome
Z <- subset(data,CHROM == "CM036387.1")
fstsubset<-Z[complete.cases(Z),]
test3 <- subset(fstsubset, WEIGHTED_FST >= 0)
Y <- test3$WEIGHTED_FST
map <- test3$BIN_START
#give variables
pdf("spline-Wstat-CM036387.1.pdf")
...
write.table(R,"CM036387.1-Fst-GeneWin.txt", quote=F,row=F)
