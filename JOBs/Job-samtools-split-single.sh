...
for file in *.aln.sam.sort.bam ; do samtools idxstats $file | cut -f 1 | grep -w `head -n # AUTOSOMES-chr.txt | tail -n 1` | xargs samtools view -b $file > ./AUTOSOMES-CHR/$file.AUTOSOMES.bam; done
